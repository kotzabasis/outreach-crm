...
### LinkedIn Lead Gen Forms — direct Lead Sync API integration

**This gate is tighter than Meta's — read this before committing to it.** LinkedIn's Lead Sync API is a separate, application-based partner program, not a self-serve review: access to LinkedIn's regular Advertising API does **not** automatically grant it. LinkedIn has historically been more selective about approving this than Meta is with `leads_retrieval`, sometimes favoring established ad-tech vendors, with no fixed timeline or guaranteed outcome. If you'd rather not go through that, the generic webhook above already works today via Zapier/Make's native LinkedIn Lead Gen Forms connector, or via LinkedIn Campaign Manager's manual CSV export paired with the existing CSV upload — either is a legitimate permanent choice, not just a stopgap.

If you're proceeding anyway:

1. Create a LinkedIn Developer App at [linkedin.com/developers/apps](https://www.linkedin.com/developers/apps/new). It must be associated with a **verified** LinkedIn Company Page (a super admin of that page has to verify the app from their own LinkedIn account) — a fake or test page will get the access request rejected outright.
2. **Auth tab** — add the redirect URL `https://your-api.onrender.com/integrations/linkedin/callback`, then copy the **Client ID**/**Client Secret** into this backend's `LINKEDIN_CLIENT_ID`/`LINKEDIN_CLIENT_SECRET` env vars. Set `LINKEDIN_REDIRECT_URI` to that same URL.
3. **Products tab** — under Additional available products, find **Lead Sync API** and click **Request Access**. You'll need: a business email address (personal addresses are rejected) that can receive a verification email, your organization's legal name/registered address/website, and a written use-case description. Review can take anywhere from days to several weeks, and can be denied (e.g. if LinkedIn can't verify your organization) or rejected (e.g. insufficient detail) — see LinkedIn's own [Getting Access to Lead Sync](https://learn.microsoft.com/en-us/linkedin/marketing/lead-sync/getting-access-leadsync) page for the current review criteria.
4. Once approved, LinkedIn provisions your app with the `r_marketing_leadgen_automation` permission (this backend also requests `r_organization_admin`). Until then, the OAuth "Σύνδεση LinkedIn" button in the Integrations tab will still complete and save a connection, but lead calls will fail with a 403 — expected, not a bug.
5. Unlike Meta, LinkedIn's OAuth is a standard consent-screen flow, so there's a real "Σύνδεση LinkedIn" button (no manual token paste). After the redirect back, you'll be asked for the **Organization URN** (e.g. `urn:li:organization:12345`) to attach the connection to — LinkedIn doesn't provide a way to auto-list "your organizations" without the same approval this section is about, so this one field is entered manually. Find your organization's numeric id via Campaign Manager's URL when viewing that account.
6. This backend registers a webhook (`/integrations/linkedin/webhook`) for real-time notifications right after you connect, and separately runs a periodic reconciliation poll (every 30 min, see `lib/scheduler.js#linkedinReconciliationTick`) as a backstop — LinkedIn's own docs note webhook deliveries can occasionally be missed or duplicated, so the poll exists to catch anything the webhook missed rather than replace it.
7. LinkedIn only issues a refresh token to apps with its separate "Programmatic Refresh Tokens" product; without it, the access token has to be re-authorized (click "Σύνδεση LinkedIn" again) before/after it expires — the Integrations tab shows "χρειάζεται επανασύνδεση" once that happens, same UX as an expired Gmail connection.

One more caveat worth being direct about: the exact webhook payload shape and lead-fetch endpoint in `src/lib/linkedinLeads.js` are built from LinkedIn's public documentation, not a live test — this can't be exercised end-to-end until your app has real Lead Sync API access. LinkedIn has changed this API's exact shape before (see their "Migration Guide for Lead Sync APIs"), so once access comes through, sanity-check a real webhook payload against what the code expects before relying on it for real leads.
